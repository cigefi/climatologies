...
def plotData(data2D,label,path,name):
    ...
    try:
        ...
        DPI = 1000
        plt.savefig(path+name+'-contourf2-'+str(DPI)+'.png', format='png', dpi=DPI)
        plt.close(fig)
        print 'Map saved'
    except:
...
if len(sys.argv) < 1:
    ...
    for f in sorted(files):
        print 'Plotting: %s' %f
        #fid = codecs.open(files[f],encoding='utf-8')
        #data2D = np.loadtxt(fid,delimiter=',')
        data2D = np.loadtxt(files[f],delimiter=',')
        #data2D = np.array(unicode(data2D))#.decode('utf-8')
        if pType == 'tas':
            ds= u'\N{DEGREE SIGN}'
            plotData(data2D,'Temperature ('+ds+'C/day)',path,f)
        else:
